...
class ModelNotFoundError(RuntimeError):
    """Raised when Anthropic returns 404 for the requested model."""


def _anthropic_error_detail(exc: Exception) -> str:
    """Extract Anthropic error detail text where available."""
...
def _create_message(client: anthropic.Anthropic, model: str, messages: list[dict]):
    """Create an Anthropic message with model-not-found normalization."""
    try:
        ...
        if getattr(exc, "status_code", None) == 404:
            ...
            raise ModelNotFoundError(
                f"Anthropic model not found (404) for --model '{model}'. "
                "The model ID is invalid or unavailable for this API key. "
                "Use a valid model ID or omit --model to use your default. "
                f"Anthropic message: {detail}"
            ) from exc
        raise
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    if len(description) > 1024:
        ...
        shorten_response = _create_message(
            client=client,
            model=model,
            messages=[
